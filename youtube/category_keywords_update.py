...
json_file_path = os.path.join(DATA_DIR, "category_keywords.json")

with open(json_file_path, "r", encoding="utf-8") as file:
    data = json.load(file)

# 데이터를 MongoDB에 저장 (카테고리별 키워드 랭킹)
formatted_data = [
    {"category": category, "ranked_keywords": [{"순위": i + 1, "키워드": keyword} for i, keyword in enumerate(keywords)]}
